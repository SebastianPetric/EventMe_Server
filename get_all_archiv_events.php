...
if($result= $db->prepare("SELECT * FROM event INNER JOIN event_user ON event.event_id= event_user.event_id AND event_user.user_id=:admin_id AND event.status=$status_open ORDER BY event.date")){
    $db->beginTransaction();
